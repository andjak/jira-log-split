...
function iso(d: string) { return new Date(`${d}T00:00:00.000Z`); }
function dateFromJql(jql: string) {
  const m1 = jql.match(/updated >= "(\d{4}-\d{2}-\d{2})"/);
  const m2 = jql.match(/updated <= "(\d{4}-\d{2}-\d{2})"/);
...
describe('IssueProviderService period delta querying', () => {
  ...
  beforeEach(() => {
    ...
    jiraApiServiceMock.fetchIssuesMinimalPaged = vi.fn(async (_jql: string, onPage: (issues: JiraIssue[], idx: number, pageSize: number) => any) => {
      await onPage([], 0, 100);
      return [];
    });
    (jiraApiServiceMock.fetchIssuesDetailedByKeys as any).mockResolvedValue([]);

    settingsServiceMock = { get: vi.fn(async (k: string) => (k === 'pipelinedPhase2Enabled' ? true : (k === 'includedProjects' ? [] : (k === 'issueSource' ? 'activity' : null)))) } as unknown as SettingsService;
  ...
  it('expanding last week -> last month queries only earlier 3 weeks delta', async () => {
    ...
    await service.getIssues({ start: iso('2023-06-24'), end: iso('2023-06-30') });
    expect(jiraApiServiceMock.fetchIssuesMinimalPaged).toHaveBeenCalledTimes(1);
    const jql1 = jiraApiServiceMock.fetchIssuesMinimalPaged.mock.calls[0][0] as string;
    ...
    await service.getIssues({ start: iso('2023-06-01'), end: iso('2023-06-30') });
    expect(jiraApiServiceMock.fetchIssuesMinimalPaged).toHaveBeenCalledTimes(1);
    const jql2 = jiraApiServiceMock.fetchIssuesMinimalPaged.mock.calls[0][0] as string;
  ...
  it('non-overlapping switch queries full new period', async () => {
    ...
    expect(jiraApiServiceMock.fetchIssuesMinimalPaged).toHaveBeenCalledTimes(1);
    jiraApiServiceMock.fetchIssuesMinimalPaged.mockClear();
    await service.getIssues({ start: iso('2023-05-01'), end: iso('2023-05-31') });
    expect(jiraApiServiceMock.fetchIssuesMinimalPaged).toHaveBeenCalledTimes(1);
    const jql = jiraApiServiceMock.fetchIssuesMinimalPaged.mock.calls[0][0] as string;
  ...
  it('remembers earlier periods and only queries newly uncovered tail', async () => {
    ...
    await service.getIssues({ start: iso('2023-06-15'), end: iso('2023-07-15') });
    expect(jiraApiServiceMock.fetchIssuesMinimalPaged).toHaveBeenCalledTimes(1);
    const jql = jiraApiServiceMock.fetchIssuesMinimalPaged.mock.calls[0][0] as string;
    const d = dateFromJql(jql);
  ...
  it('extending period end queries only new tail', async () => {
    await service.getIssues({ start: iso('2023-06-01'), end: iso('2023-06-30') });
    jiraApiServiceMock.fetchIssuesMinimalPaged.mockClear();
    await service.getIssues({ start: iso('2023-06-15'), end: iso('2023-07-31') });
    expect(jiraApiServiceMock.fetchIssuesMinimalPaged).toHaveBeenCalledTimes(1);
    const jql = jiraApiServiceMock.fetchIssuesMinimalPaged.mock.calls[0][0] as string;
    const d = dateFromJql(jql);
    expect(d.start).toBe('2023-07-01');
  ...
  it('expansion around previous period queries both leading and trailing deltas', async () => {
    ...
    await service.getIssues({ start: iso('2023-05-01'), end: iso('2023-07-31') });
    expect(jiraApiServiceMock.fetchIssuesMinimalPaged).toHaveBeenCalledTimes(2);
    const call1 = dateFromJql(jiraApiServiceMock.fetchIssuesMinimalPaged.mock.calls[0][0] as string);
    const call2 = dateFromJql(jiraApiServiceMock.fetchIssuesMinimalPaged.mock.calls[1][0] as string);
  ...
  it('shrinking to an already covered period returns computed results from cache without new queries', async () => {
    ...
    const julyIssue: JiraIssue = {
      id: '1',
      key: 'JUL-1',
      fields: { summary: '', issuetype: { iconUrl: '', name: 'Task' }, project: { key: 'P', name: 'P' }, updated: '' },
      changelog: { histories: [ { author: { accountId: me, displayName: 'Me' }, created: '2025-07-15T10:00:00.000Z', items: [ { field: 'status', fieldtype: 'jira', fromString: 'To Do', toString: 'In Progress' } ] } ] }
    } as JiraIssue;
    ...
    (jiraApiServiceMock.fetchIssuesDetailedByKeys as any).mockResolvedValue([julyIssue]);
    (jiraApiServiceMock as any).fetchIssuesMinimalPaged.mockImplementationOnce(async (_jql: string, onPage: (issues: JiraIssue[], idx: number, pageSize: number) => any) => {
      await onPage([{ id: '1', key: 'JUL-1', fields: { summary: '', issuetype: { iconUrl: '', name: 'Task' }, project: { key: 'P', name: 'P' }, updated: '' } } as any], 0, 100);
      return [];
    });

    await service.getIssues({ start: iso('2025-07-01'), end: iso('2025-08-13') });
    ...
    const res = await service.getIssues({ start: iso('2025-07-01'), end: iso('2025-07-31') });
    expect(jiraApiServiceMock.fetchIssuesMinimalPaged).not.toHaveBeenCalled();
